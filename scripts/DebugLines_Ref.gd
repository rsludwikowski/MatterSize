	...
	for planet in planets:
		...
		for i in steps:
			if i == 0:
				continue
			if i%5 == 0:
				DebugDraw3D.draw_line(planet.Pos[i-5]-planets[planetIndex].Pos[i-5]+ referenceBody.global_position,planet.Pos[i]-planets[planetIndex].Pos[i]+referenceBody.global_position,Color(1,1,0))
